...
<c:choose>
                        <c:when test="${not empty ordersDetailMap[orders.imp]}">
                            <c:forEach var="detail" items="${ordersDetailMap[orders.imp]}">
                                <tr>
                                    <td>${detail.name}</td>
                                    <td>${detail.count}개</td>
                                    <td><fmt:formatNumber value="${detail.price}" type="number" groupingUsed="true" />원</td>
                                    <td><fmt:formatNumber value="${detail.price * detail.count}" type="number" groupingUsed="true" />원</td>
                                </tr>
                            </c:forEach>
                        </c:when>
                        <c:otherwise>
                            <tr><td colspan="4">상품 정보가 없습니다.</td></tr>
                        </c:otherwise>
                    </c:choose>
                                </tbody>
                            </table>
